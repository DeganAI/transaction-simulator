...
honoApp.all('/entrypoints/simulate-transaction/invoke', async (c) => {
  ...
  if (!paymentHeader && acceptHeader.includes('text/html')) {
...
<head>
  ...
  <meta property="og:url" content="https://transaction-simulator-production.up.railway.app/entrypoints/simulate-transaction/invoke">
  ...
  <meta property="twitter:url" content="https://transaction-simulator-production.up.railway.app/entrypoints/simulate-transaction/invoke">
  ...
  <style>
    body { font-family: system-ui; max-width: 800px; margin: 40px auto; padding: 20px; background: #0a0a0a; color: #e0e0e0; }
    h1 { color: #6de8a5; }
    .info { background: #1a1a1a; padding: 15px; border-radius: 8px; margin: 15px 0; border-left: 4px solid #6de8a5; }
    code { background: #2a2a2a; padding: 3px 8px; border-radius: 4px; color: #76ad8b; }
    .field { margin: 10px 0; }
    .required { color: #ff6b6b; font-size: 0.85em; }
  </style>
</head>
<body>
  <h1>⚡ Transaction Simulator</h1>
  <p>Predict transaction outcomes before execution</p>

  <div class="info">
    <h3>x402 Payment Required</h3>
    <p>Price: <strong>$0.03 USDC</strong> per simulation</p>
    <p>Network: <strong>Base</strong></p>
  </div>

  <h2>Input Fields</h2>
  <div class="field">
    <code>chain_id</code> <span class="required">*required</span>
    <p>Chain ID (1=Ethereum, 56=BSC, 137=Polygon, 42161=Arbitrum, 10=Optimism, 8453=Base, 43114=Avalanche)</p>
  </div>
  <div class="field">
    <code>from_address</code> <span class="required">*required</span>
    <p>Sender address (0x...)</p>
  </div>
  <div class="field">
    <code>to_address</code> <span class="required">*required</span>
    <p>Recipient/contract address (0x...)</p>
  </div>
  <div class="field">
    <code>value</code>
    <p>Value to send in hex wei (optional)</p>
  </div>
  <div class="field">
    <code>data</code>
    <p>Transaction data for contract calls (optional)</p>
  </div>

  <h2>Output</h2>
  <p>Returns gas estimates, cost predictions, asset changes, and warnings</p>

  <p><small>Powered by agent-kit + viem | DeganAI</small></p>
</body>
</html>`);
  }
